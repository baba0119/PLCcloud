...
const ColSettingPanel: VFC<Props> = ({ side }) => {
  const { displayState, colSettingFunc } = useContext(LadderDisplayContext);
    ...
    <PanelArea>
      <ButtonAreaParent>
        {( x < 0 ? (
          ...
          <ColSetButton onClick={() => colSettingFunc(upColSetting)}>
            {( isUpCol ? "del" : "conn")}
          </ColSetButton>
        ))}
      </ButtonAreaParent>
      <ButtonAreaParent>
        {( x < 0 ? (
          ...
          <ColSetButton onClick={() => colSettingFunc(DownColSetting)}>
            {( isDownCol ? "del" : "conn")}
          </ColSetButton>
        ))}
      </ButtonAreaParent>
